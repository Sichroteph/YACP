...
def patch_jpegdec(env):
    ...
    # Only the active environment participates in this build. Walking every
    # cached libdeps directory lets a stale simulator checkout break an
    # unrelated firmware build even though PlatformIO will not link it.
    env_dir = env.subst("$PIOENV")
