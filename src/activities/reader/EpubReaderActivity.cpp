...
void EpubReaderActivity::silentIndexNextChapterIfNeeded(const uint16_t viewportWidth, const uint16_t viewportHeight) {
  ...
  const int readerFontId = SETTINGS.getReaderFontId();
  {
    static_assert(sizeof(Section) <= 256, "Section exceeds the optional pre-index stack budget");
    // Section is 60 bytes on this ESP32-C3 build and remains below the small-stack threshold.
    // Keep the cache probe scoped so its HalFile is destroyed before the same path may be rebuilt.
    Section cachedNextSection(epub, nextSpineIndex, renderer, sectionCacheSuffixForRenderMode(selectedRenderMode));
    if (cachedNextSection.loadSectionFile(
  ...
  for (uint8_t i = 0; i < fallbackCount && !buildSucceeded; ++i) {
    ...
    buildSucceeded = attemptSection.createSectionFile(
        ...
        SETTINGS.paragraphAlignment, viewportWidth, viewportHeight, SETTINGS.hyphenationEnabled, profile.embeddedStyle,
        SETTINGS.imageRendering, profile.bionicReadingEnabled, profile.guideReadingEnabled, nullptr, nullptr,
        &layoutAbortedForLowMemory, profile.renderMode);
    if (buildSucceeded) {
      usedRenderMode = profile.renderMode;
...
bool EpubReaderActivity::saveProgress(int spineIndex, int currentPage, int pageCount) {
  ...
  if (!epub || !EpubReaderUtils::saveProgress(*epub, spineIndex, currentPage, pageCount)) {
    return false;
  }
  const uint32_t positionKey = (static_cast<uint32_t>(spineIndex) << 16) | static_cast<uint16_t>(currentPage);
  progressSaveDebouncer.markPersisted(positionKey);
  return true;
}
...
bool EpubReaderActivity::queueProgressSave(const int spineIndex, const int currentPage, const int pageCount) {
  ...
  const uint32_t positionKey = (static_cast<uint32_t>(spineIndex) << 16) | static_cast<uint16_t>(currentPage);
  if (!progressSaveDebouncer.observe(positionKey)) {
    return true;
  }
...
bool EpubReaderActivity::flushQueuedProgress() {
  ...
  const int pageCount = spineIndex == currentSpineIndex ? section->pageCount : 0;
...
void EpubReaderActivity::drawClippingHighlights(const Page& page, const int fontId, const int orientedMarginTop,
  ...
  forEachVisiblePageWord(page, [&](const uint16_t pageWordIndex, const PageLine& line, const TextBlock& block,
    ...
    if (wordIndex + 1 < block.wordCount()) {
      const uint16_t nextIndex = wordIndex + 1;
...
void EpubReaderActivity::renderStatusBar() const {
  ...
  GUI.drawStatusBar(renderer, bookProgress, currentPage, pageCount, title, 0, textYOffset, bookmarked, timeLeft,
                    ...
                    static_cast<int>(referencePageCount), !activeFootnotePreview);
