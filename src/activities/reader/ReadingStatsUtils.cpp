...
namespace {
constexpr std::array<StrId, 12> MONTH_NAME_IDS = {
...
void formatReadingStatsShortDate(const ReadingStatsDate& date, char* buf, const size_t len) {
  ...
  snprintf(buf, len, "%s %u", I18N.get(MONTH_NAME_IDS[date.month - 1]), static_cast<unsigned>(date.day));
}
...
void formatReadingStatsMonthToken(const ReadingStatsDate& date, char* buf, const size_t len) {
  ...
  snprintf(buf, len, "%s", I18N.get(MONTH_NAME_IDS[date.month - 1]));
}
