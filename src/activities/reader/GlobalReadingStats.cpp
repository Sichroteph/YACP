...
bool GlobalReadingStats::resetLocal() {
  if (!saveToFile(GlobalReadingStats{}, GLOBAL_STATS_PATH, nullptr)) {
    return false;
  }
  return DailyReadingHistory::reset();
}
...
bool GlobalReadingStats::hasReadingOnDay(const uint32_t dayIndex) const {
  if (dayIndex > readingHistoryAnchorDay) {
    return false;
  }
  const uint32_t ageDays = readingHistoryAnchorDay - dayIndex;
  if (ageDays >= READING_HISTORY_DAYS) {
    return false;
  }
  return (readingHistoryBits[ageDays / 8u] & static_cast<uint8_t>(1u << (ageDays % 8u))) != 0;
}
