...
bool ActivityManager::canEnterReaderIdlePowerSaving() const {
  if (!currentActivity || !currentActivity->allowReaderIdlePowerSaving() || currentActivity->preventAutoSleep() ||
      pendingActivity || pendingAction != PendingAction::None || requestedUpdate.load(std::memory_order_acquire) ||
      renderInProgress.load(std::memory_order_acquire)) {
    return false;
  }

  return requestedRenderGeneration.load(std::memory_order_acquire) ==
